...
token counts. The headline `make demo` replays a recorded run with no key.

## Run

- `make demo` replays a recorded transcript. No key, no beta.
...
- `tour/replay.py` prints `transcript.md` for `make demo`.
- `transcript.md` is a recorded real run, the receipts behind `make demo`.
