...
============================================
The Agent SDK version of the repo doctor, run after Acts 1-4. It lets Claude
inspect this repo, run tests, and propose concrete improvements, with the SDK
running its own tool loop instead of a single call. This is the surface to reach
for when a task is genuinely agentic, not a single request.
...
3. Identify the top three changes that would make the demo clearer, safer, or more production-realistic. Give each as problem, then why it matters, then the fix.
