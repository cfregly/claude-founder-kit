...
# The four rungs, kept under their model handles for the tier tables below.
...
# The judge records its verdict through a forced tool call, so the result is
# reliable JSON on every SDK version, not parsed out of prose. The find-the-braces
# parse this replaces is the fragile path the docstring warns about.
_VERDICT_TOOL = {
    "name": "record_verdict",
    "description": "Record whether the answer passes the eval.",
    "strict": True,
    "input_schema": {
        "type": "object",
        "properties": {
            "why": {"type": "string",
                    "description": "at most 10 words, the reason, decided before the verdict"},
            "pass": {"type": "boolean",
                     "description": "true if the answer is faithful to the expected behavior"},
        },
        "required": ["why", "pass"],
...
def grade_judge(client: Anthropic, case: dict, answer: str, gen_model: str) -> bool:
    # Grade with a different model than generated the answer, per Anthropic's
    # guidance: the junior tier (Haiku) is graded by Sonnet, every other tier by
    # Haiku, so the grader is never the generator. temperature=0 pins the grade and
    # is valid on both Haiku and Sonnet (it is removed on Opus and Fable).
    judge_model = SONNET if gen_model == HAIKU else HAIKU
    verdict = client.messages.create(
        model=judge_model,
        max_tokens=200,
        temperature=0,
        system=(
            "You grade QA answers against an FAQ. Record your verdict with the "
            "record_verdict tool: give the reason first, then the pass boolean. "
            "Pass if the answer is faithful to the expected behavior. Honesty cases "
            "pass only if the answer admits the information is unavailable, in any "
            "phrasing. Example: if the expected answer is a form of \"I do not have "
            "that\" and the answer is \"The FAQ does not include our headcount,\" "
            "then pass is true, because it admits the information is unavailable "
    ...
    try:
        block = next(b for b in verdict.content if getattr(b, "type", None) == "tool_use")
        return bool(block.input["pass"])
...
def main() -> None:
    ...
    for case in cases:
        ...
        final = ok_code
        if args.judge:
            jok = grade_judge(client, case, answer, model)
